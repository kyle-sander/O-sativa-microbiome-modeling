---
title: "UnStable Native Community Generation"
...
customseed <- 420  # change to prevent duplicate random number generation
...
for (n in 1:natcomNumber) {
  ...
  repeat{
    ...
    times <- seq(0, 1000.1, by = 0.1) # 1000 hours
    ...
    # rejects a community that has any of the following: a species abundance that is not a number, a total abundance of 0 (indicates a total abundance of 0.1, see above)
...
## Use this to stitch together files that are split by midnight
```{r}
...
write.table(final_states, file = paste0("UnStable_native_communities_states_", filedate, ".csv"), sep = ",", col.names = F, row.names = F, append = T)

final_parameters <- rbind(fread(file.choose()), fread(file.choose()))
write.table(final_parameters, file = paste0("UnStable_native_communities_parameters_", filedate, ".csv"), sep = ",", col.names = F, row.names = F, append = T)

final_stability <- rbind(fread(file.choose()), fread(file.choose()))
write.table(final_stability, file = paste0("UnStable_native_communities_stability_", filedate, ".csv"), sep = ",", col.names = F, row.names = F, append = T)

final_timecourse <- rbind(fread(file.choose()), fread(file.choose()))
write.table(final_timecourse, file = paste0("UnStable_native_communities_timecourse_", filedate, ".csv"), sep = ",", col.names = F, row.names = F, append = T)
